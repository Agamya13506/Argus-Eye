...
sys.path.append(
    os.path.abspath(os.path.join(os.path.dirname(__file__), "../fraudshield-ml"))
)
...
_velocity_rules = {"card_testing_limit": 5, "smurfing_limit": 10, "time_window": 60}
...
_features_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../fraudshield-ml/models/features.json")
)
...
async def score_txn(request: Request):
    ...
    txn_id = txn["txn_id"]
    skip_explain = txn.get("skip_explain", False)
...
def explain_txn(txn_id: str):
    if txn_id not in _score_cache:
        raise HTTPException(
            status_code=404,
            detail="Transaction not scored yet. Call POST /score first.",
        )
...
def get_forecast():
    forecast_path = os.path.abspath(
        os.path.join(
            os.path.dirname(__file__), "../fraudshield-ml/models/forecast_7day.json"
        )
    )
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to load forecast data: {str(e)}"
        )
...
def build_network_graph(account_id: str):
        ...
        nodes.append(
            {
                "id": node,
                "risk_score": round(
                    pagerank_scores.get(node, 0.0) * 100, 2
                ),  # Using PR scaled as risk proxy
    ...
    for u, v, data in G.edges(data=True):
        if u in valid_nodes and v in valid_nodes:
            edges.append({"source": u, "target": v, "amount": data["amount"]})
...
def retrain_model(request: RetrainRequest):
    ...
    model_path = os.path.abspath(
        os.path.join(
            os.path.dirname(__file__), "../fraudshield-ml/models/lr_live_retrain.pkl"
        )
    )
    features_path = os.path.abspath(
        os.path.join(
            os.path.dirname(__file__), "../fraudshield-ml/models/features.json"
        )
    )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to load required assets: {str(e)}"
        )
...
def update_velocity_rules(rules: VelocityRulesRequest):
    ...
    return {"updated": True, "rules": _velocity_rules}
