...
export default function Compliance() {
  ...
  const exportRBIReport = () => {
    ...
    doc.text('ARGUS EYE', 14, 20);

    doc.setFontSize(14);
    doc.setTextColor(50, 50, 50);
    doc.text('Compliance & Audit Summary Report', 14, 30);
    ...
    autoTable(doc, {
      startY: 45,
      head: [['Framework', 'Score', 'Status', 'Last Audit', 'Next Audit']],
      body: complianceFrameworks.map(f => [f.name, `${f.score}%`, f.status, f.lastAudit, f.nextAudit]),
      theme: 'grid',
    ...
    autoTable(doc, {
      ...
      head: [['Audit ID', 'Framework', 'Date', 'Auditor', 'Result', 'Score']],
      body: recentAudits.map(a => [a.id, a.framework, a.date, a.auditor, a.result, `${a.score}%`]),
      theme: 'grid',
      headStyles: { fillColor: [244, 63, 94] }
    });
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text('Generated by Argus Eye', pageWidth / 2, doc.internal.pageSize.getHeight() - 10, { align: 'center' });
    }

    doc.save(`rbi-compliance-report-${new Date().toISOString().split('T')[0]}.pdf`);
  };
    ...
    >
      <header className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p style={{ color: 'var(--muted)' }}>Automated reporting, audit trails, and regulatory tracking. Fully SEBI and RBI compliant.</p>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={exportFrameworkReport} className="btn-secondary flex items-center gap-2 text-xs">
            <Download className="w-4 h-4" /> CSV
          </button>
          <button onClick={exportRBIReport} className="btn-accent flex items-center gap-2">
            <FileDown className="w-4 h-4" /> Export RBI Report
          </button>
        </div>
      </header>
